---
name: ui-craft
description: House UI craft bar for polish, component details, and animation decisions. Load when checking screens, components, layout, or motion. Parent or design-verifier loads this. Do not use during TDD Red.
disable-model-invocation: true
---

# UI Craft

House craft bar for UI polish, component details, and animation decisions. Precise motion values also live in `review-animations` STANDARDS. Load this when checking or building UI.

## When not to run

Do not run during TDD Red (writing tests). After UI exists, `design-verifier` uses this bar. After `design-verifier` Rejected twice, stop. The parent asks the user. Do not keep reviewing in a loop.
...
## Toast and dynamic-list rules

Apply these to toasts and other rapidly triggered UI:

1. **Handle edge cases invisibly.** Pause toast timers when the tab is hidden. Fill gaps between stacked toasts with pseudo-elements to maintain hover state. Capture pointer events during drag.
...
3. **Ship strong defaults.** Default easing, timing, and visual design should be excellent. Most users never customize.

### Cohesion matters

Match motion to the component and the rest of the product. A playful component can be bouncier. A professional dashboard should be crisp and fast.
