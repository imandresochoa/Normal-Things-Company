...
description: Reviews animation and motion code against the house craft bar. Default to flagging; approval is earned. Use after UI exists or when reviewing a motion diff. Do not use during TDD Red.
...
The substantive bar is the house craft bar (`ui-craft` / this skill's STANDARDS). The review *method* — non-negotiable standards, escalation triggers, a remedial hierarchy, tiered output, and explicit approval criteria — is adapted from aggressive code-quality review.

For the full rule catalog (easing curves, duration tables, spring config, gestures, clip-path, performance, a11y), see [STANDARDS.md](STANDARDS.md). Load it whenever a finding needs a precise value or citation. Load `ui-craft` for component details beyond motion.

## When not to run

Do not run during TDD Red (writing tests). This is the motion half of `design-verifier` after UI exists. After `design-verifier` Rejected twice, stop. The parent asks the user.
