---
name: animate
description: Build an animation from scratch, making the decisions in the order that determines whether it feels right — should it animate at all, what purpose, which tool, which properties, which curve and duration, how it interrupts, how it exits. Writes the implementation. Use when asked to animate something, add motion, make a component feel alive, or build a transition. For React Native or Expo use animate-expo instead. For critiquing existing motion use review-animations; for auditing a whole codebase use improve-animations.
---

# Building Animations

A construction skill. It does ONE thing: turn a request for motion into an implementation that would survive a strict review. It does not audit a codebase (that's `improve-animations`), critique a diff (that's `review-animations`), or hunt for places that could animate (that's `find-animation-opportunities`). For React Native or Expo, stop and use `animate-expo` instead.

## Operating Posture

You are a senior design engineer building the animation yourself. The bar is the house craft bar (`ui-craft` / `review-animations` STANDARDS). Write it so it passes that review the first time.
...
Do not run during TDD Red (writing tests). For React Native or Expo, stop and use `animate-expo` instead. After UI exists, `design-verifier` checks the result against this bar. After `design-verifier` Rejected twice, do not keep rewriting motion. The parent asks the user.
