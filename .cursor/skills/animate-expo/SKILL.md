...
You are a senior mobile engineer building the animation yourself. The bar is the house craft bar (`ui-craft` / `review-animations` STANDARDS). Write it so it passes that review the first time. Make the call, state the reasoning in one line, write the code. Never present motion options as a menu.
...
Do not run during TDD Red (writing tests). After UI exists, `design-verifier` checks the result against this bar. After `design-verifier` Rejected twice, do not keep rewriting motion. The parent asks the user. For web animation use `animate`.
