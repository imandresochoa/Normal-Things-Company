...
The bar is the house craft bar (`ui-craft` / `review-animations` STANDARDS). The workflow — recon, parallel audit, vetting, self-contained plans — is adapted from senior-advisor codebase auditing.
...
Do not run during TDD Red (writing tests). This skill is read-only. It does not implement. After UI exists, `design-verifier` checks the result.
